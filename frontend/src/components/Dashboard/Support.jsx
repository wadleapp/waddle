      ...
      <div className="section-body flex-r">
        <div>
          <PieChart className="engagement-chart" width={125} height={125}>
            <Pie data={neededData} startAngle={0} endAngle={360} outerRadius={60} dataKey="value">
              {neededData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.visible ? "#F49E4C" : "#FFFFFF"} />
              ))}
            </Pie>
          </PieChart>
          <div>
          ...
          <PieChart className="engagement-chart" width={125} height={125}>
            ...
            >
              {receivedData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.visible ? "#3B8EA5" : "#FFFFFF"} />
              ))}
            </Pie>
          </PieChart>
          <div>
